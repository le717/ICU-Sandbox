...
def createsandbox():
    ...
    with CreateKeyEx(HKEY_LOCAL_MACHINE, 'Software\Mindscape') as createkey:
...
def deletesandbox():
    '''Deletes Sandbox'''
    try:
        with OpenKey(HKEY_LOCAL_MACHINE, 'Software\Mindscape', 0, KEY_ALL_ACCESS) as deletestrings: # Open root key
            DeleteKey(HKEY_LOCAL_MACHINE, 'Software\Mindscape\LEGO Island') # All game registry strings
            DeleteKey(HKEY_LOCAL_MACHINE, 'Software\Mindscape') # Delete all strings and keys created
...
def closeapp():
    ...
    try:
        with OpenKey(HKEY_LOCAL_MACHINE, 'Software\Mindscape\LEGO Island', 0, KEY_ALL_ACCESS) as findsandbox: # The sandbox still exists
